...
import (
	...
	internalHttp "github.com/wptest/internal/http"
...
func (h *httpCmd) server(cmd *cobra.Command, args []string) (err error) {
	...
	messageRepo := repositories.NewMessageRepository(*k, h.configuration)

	// init service
	deviceRepo := repositories.NewDeviceRepository(db, h.configuration)
